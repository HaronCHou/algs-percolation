...
int is_full(Perco pl, int x, int y)
{
    ...
    if (idx == -1)
        return -1;
    return qf_connected(pl->grid, 0, idx);
}
...
int perco_open(Perco pl, int x, int y)
{
    ...
    if (x != 1 && is_open(pl, x-1, y)) 
        qf_union(pl->grid, idx, xy_to_int(pl, x-1, y));
    if (x != pl->n && is_open(pl, x+1, y))
        qf_union(pl->grid, idx, xy_to_int(pl, x+1, y));
    if (y != 1 && is_open(pl, x, y-1))
        qf_union(pl->grid, idx, xy_to_int(pl, x, y-1));
    if (y != pl->n && is_open(pl, x, y+1))
        qf_union(pl->grid, idx, xy_to_int(pl, x, y+1));

    if (idx <= pl->n) qf_union(pl->grid, 0, idx);
    if (idx >= (pl->n - 1) * (pl->n) + 1)
        qf_union(pl->grid, pl->n * pl->n + 1, idx);
    return 0;
}

int is_percolated(Perco pl)
{ 
    return qf_connected(pl->grid, 0, pl->n * pl->n + 1);
}
